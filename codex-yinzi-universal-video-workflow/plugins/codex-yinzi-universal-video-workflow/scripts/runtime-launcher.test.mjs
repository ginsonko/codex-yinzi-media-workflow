...
test('occupied port, concurrent opens, changed frontend, and existing configuration recover through one runtime', async () => {
  ...
  try {
    ...
    await fs.symlink(path.resolve(path.dirname(script),'../../../../backend-node/node_modules'),path.join(source,'backend-node/node_modules'),process.platform==='win32'?'junction':'dir')
    ...
    await fs.writeFile(path.join(source,'backend-node/src/server.js'),`const http=require('node:http'),fs=require('node:fs'),path=require('node:path'),crypto=require('node:crypto');const db=new (require('better-sqlite3'))('./data/test.db');const fingerprint=crypto.createHash('sha256').update('database-path:'+path.resolve('./data/test.db').replace(/[\\\\/]+/g,'/').toLowerCase()).digest('hex').slice(0,16);http.createServer((req,res)=>{if(req.url==='/api/v1/runtime-work-status')return res.end(JSON.stringify({data:{busy:false}}));if(req.url==='/health'){res.setHeader('content-type','application/json');return res.end(JSON.stringify({status:'ok'}))}if(req.url==='/api/v1/runtime-identity'){res.setHeader('content-type','application/json');return res.end(JSON.stringify({data:{orchestration_router:true,pid:process.pid,runtime_id:'fixture-'+process.pid,database:{fingerprint},source_digest:process.env.YINZI_WORKFLOW_SOURCE_DIGEST,launch_token:process.env.YINZI_WORKFLOW_LAUNCH_TOKEN}}))}res.setHeader('content-type',req.url==='/app.js'?'application/javascript':'text/html');res.end(fs.readFileSync(path.join(process.env.WEB_DIST_PATH,req.url==='/app.js'?'app.js':'index.html')))}).listen(Number(process.env.PORT),'127.0.0.1');`)
