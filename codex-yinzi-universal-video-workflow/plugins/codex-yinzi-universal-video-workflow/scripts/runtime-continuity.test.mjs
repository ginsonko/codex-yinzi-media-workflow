...
test('skill installation survives checkout removal, refreshes both discovery roots, and preserves unrelated content',async()=>temporary(async root=>{
  ...
  const dependency=path.join(source,'codex-yinzi-universal-video-workflow/plugins/codex-yinzi-universal-video-workflow/scripts/host-adapters/node_modules/example')
  await fs.mkdir(dependency,{recursive:true})
  await fs.writeFile(path.join(dependency,'index.js'),'temporary dependency version 1')
  const first=await installSkills({projectRoot:source,skillsRoots:roots})
  await assert.rejects(fs.access(path.join(first.installed_plugin,'scripts/host-adapters/node_modules')))
  await fs.writeFile(path.join(dependency,'index.js'),'temporary dependency version 2')
  assert.equal((await installSkills({projectRoot:source,skillsRoots:roots})).digest,first.digest)
...
test('real SQLite task, node, event and preferences survive source relocation, upgrade, registry recovery and concurrent launch', {timeout:120000},async()=>temporary(async root=>{
  ...
  try {
    ...
    for(const part of ['backend-node/src','backend-node/configs','backend-node/components','frontweb/src','frontweb/public','frontweb/dist'])await fs.cp(path.join(repo,part),path.join(relocated,part),{recursive:true})
    await fs.mkdir(path.join(relocated,'scripts'),{recursive:true})
    await fs.copyFile(path.join(repo,'scripts/dependencies.json'),path.join(relocated,'scripts/dependencies.json'))
    ...
    const analysis=await api('orchestration-sessions/begin','POST',{idempotency_key:'active-analysis',user_goal:'分析草稿不阻挡升级',intent:'analyze'})
    const work=await api('runtime-work-status')
    assert.equal(work.counts.analysis>=1,true)
    ...
    assert.equal((await api(`orchestration-sessions/${analysis.session.id}`)).session.status,'draft')
    assert.equal((await api(`orchestration-sessions/${analysis.session.id}`)).session.source_context.activity.state,'working')
