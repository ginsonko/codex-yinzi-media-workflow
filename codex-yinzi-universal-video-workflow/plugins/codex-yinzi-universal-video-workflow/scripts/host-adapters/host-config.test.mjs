...
test('locateProjectRoot finds real repository root dynamically without personal path', () => {
  const root = locateProjectRoot()
  assert.ok(root, 'Project root must be found dynamically')
  assert.ok(root.endsWith('public-release'), `Expected public-release, got ${root}`)
})
