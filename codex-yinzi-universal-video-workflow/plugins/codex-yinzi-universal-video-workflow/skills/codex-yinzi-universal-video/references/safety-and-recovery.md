...
Distinguish local validation failure, definite upstream rejection, accepted/running task, completed task, cancelled task, and unknown/transport failure. Unknown must remain truthful. Explicit retries remain available within existing user authorization and budget, regardless of prior state. Preserve correlation IDs, request hashes, model/config revisions, raw code/message, and billing certainty.
...
For image requests, the durable sequence is `best-effort price estimate (unknown allowed) -> atomic request-hash reservation -> one create call -> reconcile the recorded generation/task`. If the create call times out or loses its response after reservation, keep `submission_state=uncertain`; do not silently fabricate a resolved outcome. An explicit retry archives the old attempt, reopens the same node, and uses a fresh request identity; it requires no additional confirmation. If reconciliation has no generation ID or task ID, surface the unresolved state for inspection.

For guarded video requests, the durable sequence is `saved config read -> capability and live CNY catalog read -> atomic request-hash reservation -> one local video create call -> reconcile the same generation/task/provider task -> verify local download and readable media bytes`. A local create response, provider task ID, provider completion, and local download are separate facts. Use download recovery when the existing generation only needs downloading. If the user explicitly requests a new generation, reopen the original node in any status, archive its history and use a fresh request identity. Old callbacks must not overwrite the new attempt.
...
Use the user's current task, selected model and spending instructions. Follow the main Skill's unattended preference and existing-authorization rules. Respect the user's spending instructions in planning and record available price estimates with their source. Tool cost fields are advisory; unknown price or unavailable metadata is not a reason to refuse submission. Submit once with an idempotency key and reconcile uncertain outcomes. A meaningful change outside the user's authorization requires a new decision; routine progress within it does not.
