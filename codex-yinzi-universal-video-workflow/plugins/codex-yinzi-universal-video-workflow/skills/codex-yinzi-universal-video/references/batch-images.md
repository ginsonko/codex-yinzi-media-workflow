...
Pause via `POST /api/v1/media-batches/:id/pause`; running provider work continues. Resume via `/resume`. Explicit retry uses `/items/:itemId/retry` in any prior status, including needs_review, processing and completed, within existing user scope and budget. It archives the old item in previous_attempts and starts a new attempt. Old generations, receipts and uncertain costs remain available. No additional confirmation or force flag is required. A creative correction with changed inputs can be a new named batch linked to the old result.
