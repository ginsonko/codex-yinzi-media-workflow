...
第一次接触 API，可以先看银子 API 的[新手教程中心](https://yinziapi.top/guide/)。还没有安装 Agent 的用户，可以从 [Codex 安装与配置教程](https://yinziapi.top/guide/codex/)开始。工作台的 AI 配置页和银子 API 弹窗也有这两个入口。

银子 API 是可选的推荐渠道。已有其他服务商时，直接点击 **添加配置**，填写该服务商的地址、Key 和模型即可。
...
| 3 | 复制完整的 `sk-...` 字符串，回到工作台粘贴 | 按用途填写“文本 Key”“图片 Key”“视频 Key” |

文本、图片、视频分别填写对应的 Base URL、Key 和模型，也可以来自不同站点。银子 API 的访问端点通常是 `https://api.yinziapi.top/v1`；站点给出的地址不同时，以站点为准。工作流需要按服务类型保存明确的配置，不推荐把智能路由 Key 当作三类服务的统一配置。

填写后可以点击 **读取当前 Key 模型**核对可用目录，再保存配置。目录读取失败时仍可按站点提供的模型名手动填写；保存配置本身不会生成图片或视频。
