...
function create(db, log, req) {
  const clientRequestKey = String(req.client_request_key || '').trim().slice(0, 240) || null;
  if (clientRequestKey) {
    const existing = db.prepare('SELECT id FROM image_generations WHERE client_request_key = ? AND deleted_at IS NULL').get(clientRequestKey);
    if (existing) return { ...getById(db, existing.id), reused: true };
  }
  const now = new Date().toISOString();
  ...
  const info = db.prepare(
    `INSERT INTO image_generations (storyboard_id, drama_id, scene_id, provider, prompt, negative_prompt, model, image_service_type, image_config_id, frame_type, reference_images, use_first_frame_layout_lock, size, status, task_id, client_request_key, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?)`
  ).run(
