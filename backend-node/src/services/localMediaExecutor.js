...
async function execute(request, options = {}) {
  ...
  const component = await manager.ensureComponent(op.component_id, report);
  ...
  if (op.kind === 'image') {
    ...
    writeJson(workerInput, { module_id: op.id, input_path: input, output_path: output, component_dir: component.directory, parameters: params });
    ...
    if (op.kind === 'video') {
      ...
      if (af) args.push('-af',af); args.push('-c:a','aac');
