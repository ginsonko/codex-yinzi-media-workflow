...
默认质量优先，按用户选择使用质量优先/均衡/速度优先。持续将实际执行、等待、恢复动作和结果写入工作台。同一次请求的网络恢复保持幂等；在已有授权与预算内明确重试可从任何旧状态创建新尝试，保留原记录与未知费用，不要求重复确认、force 标志或先证明未扣费。下载失败优先恢复原下载。用户允许代填 Key 时保存到本机配置，绝不复制到源码、事件、日志或 Git。
