...
默认质量优先，按用户选择使用质量优先/均衡/速度优先。持续将实际执行、等待、恢复动作和结果写入工作台。同一次请求的网络恢复保持幂等；用户明确生成或重试时，从成功、运行中、失败、取消或结果未知的旧状态都可创建新尝试，保留原记录与未知费用，不要求重复确认、force 标志或先证明未扣费。工作流的报价、次数、总时长和目录元数据只作提示，不能自行否决显式请求；宿主按用户需求安排操作。下载失败可恢复原下载，用户要求重新生成时直接创建新尝试。用户允许代填 Key 时保存到本机配置，绝不复制到源码、事件、日志或 Git。
